...
*（2014年の修士論文（清水嘉泰, 早稲田大学）で「OPLSD」として提案された手法の、
...
公開の **LIU4K-v2** コーパス（CC0）——構造豊富な Building/Street 写真123枚、
ネイティブ4K〜6K を FullHD（1920×1080）グレースケールへ縮小——を i7-8700K、AVX2、
シングルスレッドで計測。比較対象はすべて**原著者の本家実装**（von Gioi の LSD、
Suárez ら の ELSED、Akinlar & Topal の ED_Lib EDLines）を SweepLSD と**同一
コンパイラ・同一ISA ターゲット**でビルドしたものです。

| | SweepLSD (one-pass) | ELSED | EDLines (ED_Lib) | LSD |
|---|---|---|---|---|
| 1枚あたり中央値時間 | **約11 ms** | 約51 ms | 約58 ms | 約278 ms |
| …SweepLSD 比 | **1×** | 4.6× | 5.2× | 25× |
...
|---|---|---|---|---|
| **GCC 15.2 / 16.1** | MinGW | **約11 ms** | 約15 ms | 1.00× |
| GCC 8.1（2018年） | MinGW | 約14 ms | 約18 ms | 1.26× |
| Clang 22.1（llvm-mingw） | MinGW | 約15 ms | 約20 ms | 1.34× |
| **clang-cl 22.1** | MSVC | **約15 ms** | 約20 ms | 1.36× |
| MSVC 19.34（`cl`） | MSVC | 約47 ms | 約61 ms | 4.2× |

大きく外れるのは MSVC の `cl` だけです。原因は端点候補の 5×5 リング判定という1つの
...
SweepLSD の設計は清水嘉泰（2014年修士論文、国内学会で OPLSD として発表。論文自体は
